...
run_loss() {
  local dataset="$1"
  local loss="$2"
  local config_path="${ROOT_DIR}/config/test_config/nlinear/${dataset}/${loss}.yaml"
  local log_path="${RUN_LOG_DIR}/nlinear_${dataset}_${loss}.log"
...
for dataset in "${DATASETS[@]}"; do
  echo "=== nlinear / ${dataset}: l1 + l2 in parallel ==="
  run_loss "${dataset}" "l1"
  pid_l1="${LAST_PID}"
  run_loss "${dataset}" "l2"
  pid_l2="${LAST_PID}"
  ...
  echo "Completed nlinear/${dataset}: l1=${s1}, l2=${s2}"
